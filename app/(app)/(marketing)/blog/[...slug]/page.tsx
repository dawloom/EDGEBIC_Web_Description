...
import { fetchAllPosts, fetchPostBySlug, transformPayloadPostToPost } from '@/lib/api/payload-cms';
...
async function getBlogPostFromParams(props: NextPageProps) {
  ...
  if (!payloadPost) {
    console.log('[Blog Post] Post not found in Posts collection, trying Pages...');
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('[Blog Post] Pages response:', data.totalDocs, 'docs found');
        ...
        console.error('[Blog Post] Pages fetch failed:', response.status, response.statusText);
      }
    } catch (error) {
